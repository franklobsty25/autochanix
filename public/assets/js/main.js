    ...
    function addToList(event, type = 'cart') {
        ...
        fetch(`/${type}/add`, {
            ...
            .then(data => {
                ...
                Toastify({
                    text: `Item added to ${type === 'cart' ? 'cart' : 'wishlist'}!`,
                    duration: 1000,
                    gravity: "top",
                    position: "right",
                    style: {
                ...
                fetch(`/${type}/remove`, {
                    ...
                    .then(({ success, subtotal, [`${type}Count`]: count }) => {
                        if (success) {
                            ...
                            updateSubtotalAndTotal(`/${type}/subtotal`);
                            updateSubtotal(`/${type}/subtotal`);
                            ...
                            if (count === 0) {
                                const sidebarCart = document.querySelectorAll(`.${type}_select_items`);
                                if (sidebarCart.length > 0) {
                                    sidebarCart.forEach(el => {
                                        el.innerHTML = `<p class="px-3 py-3">Your ${type} is empty.</p>`;
                                    });
                                }

                                // Full Shopping Cart Page
                                const fullCart = document.querySelectorAll(`.${type}-content-wrapper`);
                                if (fullCart.length > 0) {
                                    fullCart.forEach(el => {
                                        el.innerHTML = `<div class="d-flex flex-column justify-content-center align-items-center text-center" style="min-height: 400px;">
                                                            <div>
                                                                <img src="assets/img/${type}.png" class="img-fluid" width="100" alt="" />
                                                            </div>
